...
This version incorporates fixes discovered during end-to-end neural network training verification. Key changes:

1. **Type conversion instruction semantics clarified** (Section 6.7, Appendix A). The reference emulator contained a bug where `cvt_f32_i32` (opcode 0x70, signed integer to float) and `cvt_i32_f32` (opcode 0x72, float to signed integer) conversion directions were swapped. The opcode-to-operation mapping in the specification was correct; the emulator implementation has been fixed to match. This section now includes an explicit note on the canonical direction convention: the mnemonic `cvt_A_B` means "convert from type B to type A" (destination type first).
2. **Neural network training verification added to conformance evidence** (Section 9.4). A complete two-layer neural network (784→128→10, ReLU activation, softmax cross-entropy loss, SGD optimizer) has been trained on MNIST using WAVE kernels, with gradients verified against PyTorch to be numerically close (max relative difference < 1e-4). This exercises matmul, bias add, ReLU forward/backward, softmax, cross-entropy loss, transposed matmul variants, bias gradient, and SGD update kernels in a full forward-backward-update loop.
...
| 0.4 | 2026-03-31 | Clarified type conversion naming convention (cvt_A_B: B→A). Fixed emulator CVT direction bug. Added neural network training verification (MNIST, 11 kernels, gradients match PyTorch). |
