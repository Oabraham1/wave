---
title: "WAVE Specification v0.4"
description: "WAVE ISA Specification v0.4 — Working Draft, Revised (March 31, 2026)"
---

**Version:** 0.4 (Working Draft, Revised)
...
### 1.5 Changes from v0.3

This version fixes the predicate encoding defect (Defect 4) discovered by the specification verification suite, and incorporates fixes discovered during end-to-end neural network training verification. Key changes:

1. **Predicate encoding restored** (Section 8.2). The 8-bit register field rewrite in v0.2 consumed all available bits in word0, leaving no room for predicate encoding. The assembler's `encode_predicate()` function silently returned 0, and the decoder hardcoded predicate fields to unconditional. All predicated instructions (`@p0`, `@!p1`, etc.) executed unconditionally. Bits [3:0] of word0 have been repurposed: bits [1:0] encode the predicate register index (0-3), bit [2] encodes predicate negation, and bit [3] is reserved.
2. **Memory scope moved to word1** (Section 8.2, 8.3). For instructions that need a memory scope (DeviceAtomic, fence), the scope is now encoded in word1 bits [1:0] instead of word0. This freed word0 bits for predicate encoding.
3. **New opcode: Misc (0x41)** (Section 8.4). Misc operations (mov, mov_imm, mov_sr) previously shared opcode 0x3F (Control) with a MISC_OP_FLAG in the flags field. Since the flags field no longer exists, Misc operations moved to their own opcode 0x41.
4. **SyncOp modifier offset** (Section 8.4). Sync operations (return, halt, barrier, fence) share Control opcode 0x3F with modifier values offset by +8 (`SYNC_MODIFIER_OFFSET = 8`). ControlOp uses modifiers 0-7; SyncOp uses modifiers 8-15.
5. **Type conversion instruction semantics clarified** (Section 6.7, Appendix A). The reference emulator contained a bug where `cvt_f32_i32` and `cvt_i32_f32` conversion directions were swapped. The mnemonic `cvt_A_B` means "convert from type B to type A" (destination type first).
6. **Neural network training verification added to conformance evidence** (Section 9.4). A complete two-layer neural network (784->128->10, ReLU activation, softmax cross-entropy loss, SGD optimizer) has been trained on MNIST using WAVE kernels, with gradients verified against PyTorch to be numerically close (max relative difference < 1e-4).

### 1.6 Changes from v0.2

These changes were introduced in v0.3:
...
**Naming convention (v0.4 clarification):** The mnemonic `cvt_A_B` converts from type B to type A. The destination type appears first. For example, `cvt_f32_i32` converts a signed 32-bit integer (i32) to a 32-bit float (f32). This convention is consistent with the opcode table in Appendix A.
...
Instructions are encoded as 32-bit words. Most instructions that need a second source register, an immediate value, or a memory scope use an additional 32-bit word (64 bits total). The opcode determines whether word1 is present.

### 8.2 Word0 Format (32-bit)

**v0.4 change:** Bits [3:0] of word0 have been repurposed for predicate encoding. The scope field has been moved to word1 for instructions that need it. The flags field has been eliminated.

| Bits | Field | Description |
|---|---|---|
| [31:24] | opcode | 8 bits -- operation class |
| [23:16] | rd | 8 bits -- destination register (0-255). rd==0 on atomics indicates non-returning variant. |
| [15:8] | rs1 | 8 bits -- source register 1 (0-255) |
| [7:4] | modifier | 4 bits -- instruction-specific sub-opcode (0-15) |
...
Predicate encoding: when pred_reg=0 and pred_neg=0, the instruction is unconditional (no predicate). When pred_reg is nonzero, the instruction executes only if the specified predicate register is true (or false, if pred_neg=1). For example, `@p1` sets pred_reg=1, pred_neg=0; `@!p2` sets pred_reg=2, pred_neg=1.

### 8.3 Word1 Format (32-bit, Extended)

For instructions needing additional source registers, an immediate, or a memory scope, the opcode determines that a second 32-bit word follows:
...
Alternatively, the entire word1 can serve as a 32-bit immediate value (e.g., for `mov_imm`). The opcode in word0 determines interpretation.

### 8.4 Opcode Map

The 8-bit opcode field provides 256 primary opcodes. The following table shows the actual opcode assignments as implemented:

| Range | Category |
|---|---|
| 0x00-0x0B | Integer arithmetic (iadd, isub, imul, imul_hi, imad, idiv, imod, ineg, iabs, imin, imax, iclamp) |
| 0x10-0x1F | Floating-point arithmetic: F32 (0x10-0x1A), FUnaryOps (0x1B), F16 (0x1C), F16Packed (0x1D), F64 (0x1E), F64DivSqrt (0x1F) |
| 0x20-0x27 | Bitwise operations (and, or, xor, not, shl, shr, sar, bitops) |
| 0x28-0x2C | Comparison, select, conversion (icmp, ucmp, fcmp, select, cvt) |
| 0x2D-0x2E | BF16 arithmetic (bf16ops, bf16packed) |
| 0x30-0x31 | Local memory (local_load, local_store) |
| 0x38-0x39 | Device memory (device_load, device_store) |
| 0x3C-0x3D | Atomic operations (local_atomic, device_atomic) |
| 0x3E | Wave operations (modifier selects: shuffle, shuffle_up, shuffle_down, shuffle_xor, broadcast, ballot, any, all) |
| 0x3F | Control flow and synchronization. ControlOp modifier 0-7: if, else, endif, loop, break, continue, endloop, call. SyncOp modifier 8-15 (offset by SYNC_MODIFIER_OFFSET=8): return, halt, barrier, fence_acquire, fence_release, fence_acq_rel, wait, nop. |
| 0x40 | MMA operations (modifier selects: mma_load_a, mma_load_b, mma_store_c, mma_compute) |
| 0x41 | Misc operations (modifier selects: mov, mov_imm, mov_sr) |
...
**v0.4 addition: Neural network training verification.** In addition to the unit-level conformance tests, WAVE kernels have been verified through end-to-end neural network training on MNIST. A two-layer network (784→128→10, ReLU, softmax cross-entropy, SGD) was trained using 11 WAVE kernels (matmul, bias_add, relu_forward, relu_backward, softmax_forward, cross_entropy_loss, softmax_ce_backward, matmul_at_b, matmul_a_bt, bias_grad, sgd_update). Gradients computed by the WAVE emulator match PyTorch gradients to within a max relative difference of 1e-4. The same kernels were verified on Apple Metal hardware, with emulator-to-Metal results confirmed as bit-exact or numerically close across all 11 kernels.
...
| 0.4 | 2026-04-03 | Fixed predicate encoding (Defect 4): repurposed word0 bits [3:0] for pred_reg and pred_neg. Moved scope to word1. Added Opcode::Misc (0x41). SyncOp modifier offset (+8). Clarified CVT naming convention. Fixed emulator CVT direction bug. Added neural network training verification. |
