...
class WaveNN:
    ...
    def forward(self, X_batch):
        ...
        [z1_flat] = run_kernel(
            self.kernels["matmul"],
            buffers=[X_batch.flatten(), self.W1.flatten(), z1.flatten()],
        ...
        [z1_biased_flat] = run_kernel(
            self.kernels["bias_add"],
            buffers=[z1.flatten(), self.b1, z1_biased.flatten()],
            scalars=[(M, "u32"), (N, "u32")],
            output_specs=[(2, M * N)],
            workgroup=(min(256, M * N), 1, 1),
            grid=(max(1, (M * N + 255) // 256), 1, 1),
        ...
        total1 = M * N
        [a1_flat] = run_kernel(
            self.kernels["relu_forward"],
            buffers=[z1_biased.flatten(), a1.flatten()],
            scalars=[(total1, "u32")],
            output_specs=[(1, total1)],
        ...
        [z2_flat] = run_kernel(
            self.kernels["matmul"],
            buffers=[a1.flatten(), self.W2.flatten(), z2.flatten()],
            scalars=[(M2, "u32"), (K2, "u32"), (N2, "u32")],
            output_specs=[(2, M2 * N2)],
        ...
        [z2_biased_flat] = run_kernel(
            self.kernels["bias_add"],
            buffers=[z2.flatten(), self.b2, z2_biased.flatten()],
            scalars=[(M2, "u32"), (N2, "u32")],
            output_specs=[(2, M2 * N2)],
            workgroup=(min(256, M2 * N2), 1, 1),
            grid=(max(1, (M2 * N2 + 255) // 256), 1, 1),
        )
        ...
        [sm_flat] = run_kernel(
            self.kernels["softmax_forward"],
            buffers=[z2_biased.flatten(), softmax_out.flatten()],
            scalars=[(M2, "u32"), (N2, "u32")],
            output_specs=[(1, M2 * N2)],
    ...
    def compute_loss(self, softmax_out, labels):
        ...
        [loss_vals] = run_kernel(
            self.kernels["cross_entropy_loss"],
            buffers=[softmax_out.flatten(), labels_f32, losses],
            scalars=[(batch_size, "u32"), (cols, "u32")],
            output_specs=[(2, batch_size)],
    ...
    def backward(self, labels, cache):
        ...
        [d_z2_flat] = run_kernel(
            self.kernels["softmax_ce_backward"],
            buffers=[softmax_out.flatten(), labels_f32, d_z2.flatten()],
            scalars=[(batch_size, "u32"), (cols, "u32")],
            output_specs=[(2, batch_size * cols)],
            workgroup=(min(256, batch_size), 1, 1),
            grid=(max(1, (batch_size + 255) // 256), 1, 1),
        )
        ...
        [dW2_flat] = run_kernel(
            self.kernels["matmul_at_b"],
            buffers=[a1.flatten(), d_z2.flatten(), dW2.flatten()],
            scalars=[(M_at, "u32"), (K_at, "u32"), (N_at, "u32")],
            output_specs=[(2, K_at * N_at)],
        ...
        [db2_vals] = run_kernel(
            self.kernels["bias_grad"],
            buffers=[d_z2.flatten(), db2],
            scalars=[(batch_size, "u32"), (self.output_dim, "u32")],
            output_specs=[(1, self.output_dim)],
        ...
        [d_a1_flat] = run_kernel(
            self.kernels["matmul_a_bt"],
            buffers=[d_z2.flatten(), self.W2.flatten(), d_a1.flatten()],
            scalars=[(M_abt, "u32"), (K_abt, "u32"), (N_abt, "u32")],
            output_specs=[(2, M_abt * K_abt)],
        ...
        [d_z1_flat] = run_kernel(
            self.kernels["relu_backward"],
            buffers=[d_a1.flatten(), z1_biased.flatten(), d_z1.flatten()],
            scalars=[(total_hidden, "u32")],
            output_specs=[(2, total_hidden)],
        ...
        [dW1_flat] = run_kernel(
            self.kernels["matmul_at_b"],
            buffers=[X.flatten(), d_z1.flatten(), dW1.flatten()],
            scalars=[(M_at1, "u32"), (K_at1, "u32"), (N_at1, "u32")],
            output_specs=[(2, K_at1 * N_at1)],
        ...
        [db1_vals] = run_kernel(
            self.kernels["bias_grad"],
            buffers=[d_z1.flatten(), db1],
            scalars=[(batch_size, "u32"), (self.hidden_dim, "u32")],
            output_specs=[(1, self.hidden_dim)],
    ...
    def update_weights(self, dW1, db1, dW2, db2):
        ...
        for param, grad in [
            ...
            n = param.size
            [updated] = run_kernel(
                self.kernels["sgd_update"],
                buffers=[param.flatten(), grad.flatten(), lr_buf],
                scalars=[(n, "u32")],
                output_specs=[(0, n)],
...
def train(
    ...
    model = WaveNN(input_dim=784, hidden_dim=hidden_dim, output_dim=10, lr=lr)
