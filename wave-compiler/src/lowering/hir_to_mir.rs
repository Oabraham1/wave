...
impl HirToMirLowerer {
    ...
    fn lower_assign(&mut self, target: &str, value: &Expr) -> Result<(), CompileError> {
        let val = self.lower_expr(value)?;
        let ty = self.infer_mir_type(value);
        self.variables.insert(target.to_string(), val);
        self.variable_types.insert(target.to_string(), ty);
        Ok(())
    }
    ...
    fn lower_for(
        ...
        self.variables.insert(var.to_string(), phi_dest);
        self.variable_types.insert(var.to_string(), MirType::I32);

        let loop_carried = self.build_loop_carried_vars(var);
    ...
    fn build_loop_carried_vars(
        &mut self,
        loop_var: &str,
    ) -> Vec<(String, ValueId, ValueId, MirType)> {
        let var_entries: Vec<(String, ValueId, MirType)> = self
            .variables
            .iter()
            .filter(|(name, _)| name.as_str() != loop_var)
            .map(|(name, &pre_val)| {
                let ty = self
                    .variable_types
                    .get(name)
    ...
    fn finalize_for_loop_phis(
        &mut self,
        header_block: crate::mir::value::BlockId,
        preheader_block: crate::mir::value::BlockId,
        latch_block: crate::mir::value::BlockId,
        phi_dest: ValueId,
        start_val: ValueId,
        updated: ValueId,
        loop_carried: &[(String, ValueId, ValueId, MirType)],
    ) {
    ...
    fn lower_expr(&mut self, expr: &Expr) -> Result<ValueId, CompileError> {
        match expr {
            ...
            Expr::Literal(lit) => match lit {
                #[allow(clippy::cast_possible_truncation)] // GPU literal narrowing: i64 -> i32
                Literal::Int(v) => Ok(self.builder.emit_const(ConstValue::I32(*v as i32))),
                #[allow(clippy::cast_possible_truncation)] // GPU literal narrowing: u64 -> u32
                Literal::UInt(v) => Ok(self.builder.emit_const(ConstValue::U32(*v as u32))),
                #[allow(clippy::cast_possible_truncation)] // GPU literal narrowing: f64 -> f32
            ...
            Expr::ThreadId(dim) => Ok(self.emit_special_reg_read(dim_to_thread_id_sr(*dim))),
            Expr::WorkgroupId(dim) => Ok(self.emit_special_reg_read(dim_to_workgroup_id_sr(*dim))),
            Expr::WorkgroupSize(dim) => {
                Ok(self.emit_special_reg_read(dim_to_workgroup_size_sr(*dim)))
            }
            Expr::LaneId => Ok(self.emit_special_reg_read(4)),
            Expr::WaveWidth => Ok(self.emit_special_reg_read(14)),
    ...
    fn emit_special_reg_read(&mut self, sr_index: u8) -> ValueId {
        let dest = self.builder.next_value();
        self.builder
