...
/// Compile a complete WBIN file into SYCL C++ source.
///
/// # Errors
///
...
pub fn compile(wbin_data: &[u8]) -> Result<String, CompileError> {
    ...
    for (i, kernel_info) in wbin.kernels.iter().enumerate() {
        ...
        let kernel_sycl = gen.generate(&instructions, kernel_info)?;
        output.push_str(&kernel_sycl);
        output.push('\n');
    }
...
/// Compile a single kernel's code section into a SYCL launch function.
///
/// # Errors
///
